            ...
            <ul>
                ...
                <li class="board-text"><a href="${path}/community/board">자유
                    ...
                    <ul class="community-detail-sideMenu">
                        ...
                        <li><a href="${ path }/community/board/mypet"
                            style="font-weight: bold; color: #B29254;">마이펫 자랑</a></li>
                    </ul></li>
            </ul>
        </div>
                ...
                <div>
                    <div>
                        <select name="communitySelect" id="communitySelect">
                            <option value="title"
                                <c:if test="${searchInfoMap.select == 'title'}">selected</c:if>>제목</option>
                            <option value="name"
                                <c:if test="${searchInfoMap.select == 'name'}">selected</c:if>>작성자</option>
                            <option value="content"
                                <c:if test="${searchInfoMap.select == 'content'}">selected</c:if>>내용</option>
                        </select>
                    </div>
                    <div>
                        <img src="${ path }/img/community/search.png"> <input
                            type="text" name="communitySearch" id="communitySearch"
                            placeholder="검색어를 입력해주세요." value="${ searchInfoMap.search }">
                    </div>
                    <div>
                        ...
                        <c:forEach var="board" items="${ boardMypetList }">
                            <tr>
                                <c:if test="${ board.communityCategory eq '[공지사항]' }">
                                    <td><img
                                        src="${ path }/img/community/ant-design_sound-filled.png"
                                        alt=""></td>
                                    <td class="common-text-left"
                                        style="display: block; padding-top: 20px; padding-left: 5px; border-style: none;">
                                        <a
                                        href="${ path }/community/noticedetail?no=${board.communityNo}"
                                        style="font-size: 16px;">${ board.communityCategory } ${ board.communityTitle }</a>
                                    </td>
                                ...
                                <c:if test="${ board.communityCategory ne '[공지사항]' }">
                                    ...
                                    <td class="common-text-left"
                                        style="display: block; padding-top: 20px; padding-left: 5px; border-style: none;">
                                        <a
                                        href="${ path }/community/board/mypetdetail?no=${board.communityNo}"
                                        style="font-size: 16px;">${ board.communityCategory } ${ board.communityTitle }</a>
                                    </td>
                                    <td>${ board.communityWriterId }</td>
                                    <td class="common-text-right">${ board.communityCount }</td>
            ...
            <div class="common-page-number">
                <ul>
                    ...
                    <c:if test="${ empty searchInfoMap.search }">
                        <li><a
                            href="${ path }/community/board/mypet?page=${ pageInfo.prevPage }">&lt;</a></li>
                    </c:if>
                    <c:if test="${ not empty searchInfoMap.search }">
                        <li><a
                            href="${ path }/community/board/mypet?page=${ pageInfo.prevPage }&select=${searchInfoMap.select}&search=${searchInfoMap.search}">&lt;</a></li>
                    </c:if>
                    <!-- 5개 페이지 목록 -->
                    <c:if test="${ empty boardMypetList }">
                    ...
                    <c:forEach var="current" begin="${ pageInfo.startPage }"
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                <c:if test="${ empty searchInfoMap.search }">
                                    <li><a
                                        href="${ path }/community/board/mypet?page=${ current }">${ current }</a></li>
                                </c:if>
                                <c:if test="${ not empty searchInfoMap.search }">
                                    <li><a
                                        href="${ path }/community/board/mypet?page=${ current }&select=${searchInfoMap.select}&search=${searchInfoMap.search}">${ current }</a></li>
                                </c:if>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                    ...
                    <c:if test="${ empty searchInfoMap.search }">
                        <li><a
                            href="${ path }/community/board/mypet?page=${ pageInfo.nextPage }">&gt;</a></li>
                    </c:if>
                    <c:if test="${ not empty searchInfoMap.search }">
                        <li><a
                            href="${ path }/community/board/mypet?page=${ pageInfo.nextPage }&select=${searchInfoMap.select}&search=${searchInfoMap.search}">&gt;</a></li>
                    </c:if>
                </ul>
            </div>
        </div>
