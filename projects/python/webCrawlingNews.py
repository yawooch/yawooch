...
if __name__ == "__main__":
    ...
    if not any(news_data.values()):
        ...
        current_time_folder = datetime.now().strftime("%Y%m")
        
        # 기본 경로 설정
        base_path = "D:/workspace/git_project/yawooch/projects/python/crawlResult/news"
        folder_path = os.path.join(base_path, f"naver_news_{current_time_folder}")
        ...
        # JSON 파일로 저장
        json_filename = os.path.join(folder_path, f"naver_news_{current_time}.json")
        save_to_json(news_data, json_filename)
        print(f"JSON 파일 저장 완료: {json_filename}")
        
        # Markdown 파일로 저장
        md_filename = os.path.join(folder_path, f"naver_news_{current_time}.md")
